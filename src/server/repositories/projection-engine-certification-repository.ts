...
export const projectionEngineCertificationRepository: ProjectionEngineCertificationPersistence = {
  async createReport(report: ProjectionEngineCertificationReport, actor: string): Promise<ProjectionEngineCertificationReport> {
    return withWriteTransaction("persisting immutable Projection Engine Certification report", async () => {
      const sql = getWriteSql("persisting immutable Projection Engine Certification report");
      const certificationRunId = randomUUID();
      await sql`
        INSERT INTO projection_engine_certification_runs (
          ...
          projection_statistics, failure_statistics, determinism_results,
          ...
          ${report.subjects}, ${sql.json(report.projectionStatistics as any)},
      ...
      for (const stage of report.stageResults) {
        await sql`INSERT INTO projection_engine_certification_stage_results (certification_run_id, stage, status) VALUES (${certificationRunId}, ${stage.stage}, ${stage.status})`;
      }
      for (const caseResult of report.caseResults) {
        const [caseRow] = await sql<{ caseResultId: string }[]>`
          INSERT INTO projection_engine_certification_case_results (
        ...
        `;
        for (const result of caseResult.projectionResults) {
          await sql`
            INSERT INTO projection_engine_certification_projection_results (
              case_result_id, area, passed, lineage_verified, audit_verified
            ) VALUES (${caseRow!.caseResultId}, ${result.area}, ${result.passed}, ${result.lineageVerified}, ${result.auditVerified})
          `;
        }
        for (const failure of caseResult.failureInjectionResults) {
          await sql`
            INSERT INTO projection_engine_certification_failure_results (
        ...
        for (const invariant of caseResult.invariants) {
          await sql`
            INSERT INTO projection_engine_certification_invariant_results (
