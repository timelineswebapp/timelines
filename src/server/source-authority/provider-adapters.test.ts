...
test("Library of Congress retrieval normalizes persisted HTTP canonical URLs before fetch", async () => {
  resetSourceProviderHealth();
  setSourceProviderRuntimeStoreForTests(null);
  const sourceRecord: SourceAuthorityRegistryRecord = {
  ...
  globalThis.fetch = (async (input: string | URL | Request) => {
    requestedUrl = String(input);
    return new Response("Library of Congress telephone source text.", {
      status: 200,
  ...
  try {
    const result = await sourceRetrievalService.retrieve({ sourceRecordId: "source-loc-1", actor: "provider-adapter-test" });
    assert.equal(requestedUrl, "https://www.loc.gov/item/2009633130/");
    assert.equal(result.snapshot.retrievalUrl, requestedUrl);
  } finally {
    restore();
  }
