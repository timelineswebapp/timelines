...
import { approvalStepTransitionSchema, uuidParamSchema } from "@/src/server/validation/schemas";
...
export const POST = withAdminAuth(async (request: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  return ok(await adminService.approveGovernanceApprovalStep({ ...input, id }));
});
