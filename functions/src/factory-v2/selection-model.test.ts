...
test("B1 significance evaluation is bounded, schema constrained, and has no research tool", async () => {
  ...
  const result = await evaluateHistoricalSignificance({ context: { ...TEST_CONTEXT, ...LINEAGE }, scope, researchMap: map, eligibleEvents: [event], approvedClaims: [claim], provider: { generateContent: async (request) => { requests.push(request); return { text: response(), usageMetadata: { promptTokenCount: 100, candidatesTokenCount: 50, totalTokenCount: 150 } }; } } });
...
test("B1 significance evaluation permits one semantic repair and rejects invented map references", async () => {
  let calls = 0;
  const result = await evaluateHistoricalSignificance({ context: { ...TEST_CONTEXT, ...LINEAGE }, scope, researchMap: map, eligibleEvents: [event], approvedClaims: [claim], provider: { generateContent: async () => ({ text: calls++ === 0 ? response("phase-invented") : response() }) } });
...
test("B1 model execution identity covers exact usage and timing provenance", async () => {
  const context = { ...TEST_CONTEXT, ...LINEAGE };
