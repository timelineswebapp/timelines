...
export async function evaluateHistoricalSignificance(input: {
  ...
  const inputArtifactIds = [input.scope.scopeContractId, input.researchMap.researchMapId, input.context.completedKnowledgeSetId, input.context.finalCoverageAuditId, ...input.eligibleEvents.map((event) => event.eventVersionId)];
    ...
    for (let transportAttempts = 1; transportAttempts <= 3; transportAttempts += 1) {
      ...
      try {
        ...
        try {
          ...
          const executionPayload = { artifactType: "SELECTION_MODEL_EXECUTION" as const, schemaVersion: V2_B_SCHEMA_VERSION, pipelineVersion: V2_B_PIPELINE_VERSION, policyVersion: V2_B_SELECTION_POLICY_VERSION, promptVersion: V2_B_PROMPT_VERSION, modelExecutionRef: null, parentArtifactIds: [input.scope.scopeContractId, input.researchMap.researchMapId, input.context.completedKnowledgeSetId, input.context.finalCoverageAuditId], createdAt: input.context.createdAt, corpusId: input.context.corpusId, topicId: input.context.topicId, runId: input.context.runId, generation: input.context.generation, sourceKnowledgeRunId: null, completedKnowledgeSetId: input.context.completedKnowledgeSetId, completedKnowledgeSetHash: input.context.completedKnowledgeSetHash, finalCoverageAuditId: input.context.finalCoverageAuditId, finalCoverageAuditHash: input.context.finalCoverageAuditHash, candidateInputHash: input.context.candidateInputHash, sourceKnowledgeBundle: { pipelineVersion: "factory-v2-a.13" as const, schemaVersion: "factory-v2-a.5" as const, policyVersion: "evidence-first-v2-a.11" as const, promptVersion: "factory-v2-a-prompts.8" as const }, executionMode: "SHADOW" as const, publicationEligible: false as const, governanceSubmissionAllowed: false as const, immutable: true as const, stage: "SIGNIFICANCE_SELECTION" as const, inputArtifactIds, inputHash: payloadHash({ completedKnowledgeSetHash: input.context.completedKnowledgeSetHash, candidateInputHash: input.context.candidateInputHash, scope: input.scope.payloadHash, map: input.researchMap.payloadHash, events: input.eligibleEvents.map((event) => event.payloadHash), claims: claims.map((claim) => claim.claimVersionId) }), promptHash, responseHash, validationState: repairAttempt ? "REPAIRED" as const : "VALID" as const, boundedResponse: body.slice(0, 80_000), responseTruncated: body.length > 80_000, repairAttempt, transportAttempts, usage: { inputTokens: (response as ProviderResponse).usageMetadata?.promptTokenCount ?? null, outputTokens: (response as ProviderResponse).usageMetadata?.candidatesTokenCount ?? null, totalTokens: (response as ProviderResponse).usageMetadata?.totalTokenCount ?? null, monetaryCost: null, costMeasurement: "NOT_MEASURABLE" as const }, startedAt: new Date(startedAt).toISOString(), completedAt: new Date().toISOString(), latencyMs: Date.now() - startedAt };
