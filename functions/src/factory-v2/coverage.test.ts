...
test("coverage audit and gap plan converge across execution-only reruns", () => {
  ...
  assert.equal(secondAudit.payloadHash, firstAudit.payloadHash);
  ...
  assert.equal(secondPlan.completionPlanId, firstPlan.completionPlanId);
