...
test("B1 selection policy v1/v2 artifacts coexist while execution-only reruns converge", () => {
  ...
  const lineage = { completedKnowledgeSetId: "completed-knowledge-set-test", completedKnowledgeSetHash: "a".repeat(64), finalCoverageAuditId: "final-coverage-audit-test", finalCoverageAuditHash: "b".repeat(64), candidateInputHash: "c".repeat(64) } as const;
  const build = (selectionPolicyVersion: string, runId = "b1-run") => assembleTimelineSelection({ context: { ...TEST_CONTEXT, ...lineage, runId }, scope, researchMap: map, events: [event], claims, authorityVerdicts: verdicts, conflicts: [], significance, modelExecutionRef: null, selectionPolicyVersion });
  ...
  const { completedKnowledgeSetId: _setId, completedKnowledgeSetHash: _setHash, finalCoverageAuditId: _auditId, finalCoverageAuditHash: _auditHash, candidateInputHash: _inputHash, ...legacyPayload } = v1;
  const legacy = selectionArtifactSchema.parse(attachPayloadHash({ ...legacyPayload, schemaVersion: "factory-v2-b.1", pipelineVersion: "factory-v2-b.1", sourceKnowledgeBundle: { pipelineVersion: "factory-v2-a.10", schemaVersion: "factory-v2-a.3", policyVersion: "evidence-first-v2-a.9", promptVersion: "factory-v2-a-prompts.8" } }));
